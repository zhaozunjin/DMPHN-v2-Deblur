...
import argparse
import random
import modelattentioned as models
import torchvision
...
parser.add_argument("-g","--gpu",type=int, default=5)
args = parser.parse_args()

#Hyper Parameters
METHOD = "SDNet4ed_attention"
...
def main():
    ...
    for s in ['s1', 's2', 's3', 's4']:
        ...
        for lv in ['lv1', 'lv2', 'lv3']:
            ...
            encoder_optim[s][lv] = RAdam(encoder[s][lv].parameters(),lr=LEARNING_RATE)
            decoder_optim[s][lv] = RAdam(decoder[s][lv].parameters(),lr=LEARNING_RATE)
    ...
    for epoch in range(args.start_epoch, EPOCHS):
        ...
        for iteration, inputs in enumerate(train_dataloader):            
            ...
            if (iteration+1)%10 == 0:
                stop = time.time()
                print(METHOD + "   epoch:", epoch, "iteration:", iteration+1, "loss:%.4f"%loss.item(), 'time:%.4f'%(stop-start))
                start = time.time()
        for s in ['s1', 's2', 's3', 's4']:
            for lv in ['lv1', 'lv2', 'lv3']:
                    ...
                    for s in ['s1', 's2', 's3', 's4']:
                        for lv in ['lv1', 'lv2', 'lv3']:
                            torch.save(encoder[s][lv].state_dict(),
                                       str('./checkpoints/' + METHOD + '/epoch' + str(epoch) + "/encoder_" + s + "_" + lv + ".pkl"))
                            torch.save(decoder[s][lv].state_dict(),
                                       str('./checkpoints/' + METHOD + '/epoch' + str(epoch) + "/decoder_" + s + "_" + lv + ".pkl"))
