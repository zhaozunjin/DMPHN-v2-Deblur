...
#     filename = './test_results/' + EXPDIR + "/" + "Iter_" + str(iteration) + "_deblur.png"
...
def main():
    ...
    for s in ['s1', 's2', 's3', 's4']:
        ...
        for lv in ['lv1', 'lv2', 'lv3']:
            ...
            encoder[s][lv].cuda(GPU)
            decoder[s][lv].cuda(GPU)
    ...
    for images_name in os.listdir(SAMPLE_DIR):
    	with torch.no_grad():
            ...
            # images['lv1'] = Images['blur_image']
            # images['lv1'] = Variable(Images['blur_image'] - 0.5).cuda(GPU)
