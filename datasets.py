...
class GoProDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 若用桨尖的数据则将下面两行取消注释，用GoPro数据用上面两行
        # blur_image = Image.open(
        #     os.path.join(self.root_dir, image_name[0], 'blur', image_name[2])).convert('RGB')
        # sharp_image = Image.open(
        #     os.path.join(self.root_dir, image_name[0], 'sharp',image_name[2])).convert('RGB')
