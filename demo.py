...
import argparse
import random
# import models.modelstitused as models
import torchvision
...
parser.add_argument("-m","--method",type=str, default="modelattentionted")
parser.add_argument("-sd","--sample_dir",type=str, default="test_samples")
args = parser.parse_args()

#Hyper Parameters
METHOD = args.method
SAMPLE_DIR = args.sample_dir
EXPDIR = args.method+"_res"
GPU = args.gpu

if METHOD == "modelstitused":
    import models.modelstitused as models
elif METHOD == "modelstitusted":
    import models.modelstitusted as models
elif METHOD == "modelattentioned":
    import models.modelattentioned as models
elif METHOD == "modelattentionted":
    import models.modelattentionted as models
...
def main():
    ...
    encoder_lv4 = models.Encoder().cuda(GPU)

    decoder_lv1 = models.Decoder().cuda(GPU)
    decoder_lv2 = models.Decoder().cuda(GPU)
    decoder_lv3 = models.Decoder().cuda(GPU)
    decoder_lv4 = models.Decoder().cuda(GPU)

    if os.path.exists(str('./checkpoints/' + METHOD + "/encoder_lv1.pkl")):
        encoder_lv1.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/encoder_lv1.pkl"), map_location='cuda:0'))
        print("load encoder_lv1 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/encoder_lv2.pkl")):
        encoder_lv2.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/encoder_lv2.pkl"), map_location='cuda:0'))
        print("load encoder_lv2 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/encoder_lv3.pkl")):
        encoder_lv3.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/encoder_lv3.pkl"), map_location='cuda:0'))
        print("load encoder_lv3 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/encoder_lv4.pkl")):
        encoder_lv4.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/encoder_lv4.pkl"), map_location='cuda:0'))
        print("load encoder_lv4 success")

    if os.path.exists(str('./checkpoints/' + METHOD + "/decoder_lv1.pkl")):
        decoder_lv1.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/decoder_lv1.pkl"), map_location='cuda:0'))
        print("load encoder_lv1 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/decoder_lv2.pkl")):
        decoder_lv2.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/decoder_lv2.pkl"), map_location='cuda:0'))
        print("load decoder_lv2 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/decoder_lv3.pkl")):
        decoder_lv3.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/decoder_lv3.pkl"), map_location='cuda:0'))
        print("load decoder_lv3 success")
    if os.path.exists(str('./checkpoints/' + METHOD + "/decoder_lv4.pkl")):
        decoder_lv4.load_state_dict(torch.load(str('./checkpoints/' + METHOD + "/decoder_lv4.pkl"), map_location='cuda:0'))
